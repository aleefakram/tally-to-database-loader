...
- Initial .NET parity targets MSSQL, MySQL, and PostgreSQL. These providers are implemented with split-phase database atomicity, but remain "uncertified" by default. SQLite is fully certified via fast local unit tests; MSSQL, MySQL, and PostgreSQL require explicit opt-in execution of the smoke test suite against active database engines to be certified. BigQuery, ADLS, CSV, JSON, and DuckDB are later explicit adapters, not part of the first parity gate.
