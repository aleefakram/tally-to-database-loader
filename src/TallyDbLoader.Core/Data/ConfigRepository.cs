                ...
                using (var transaction = conn.BeginTransaction())
                {
                    try
                    {
                        var parameters = new
                        {
                            ...
                            LiabilityTotal = run.LiabilityTotal.ToString("F4", System.Globalization.CultureInfo.InvariantCulture),
                            AssetTotal = run.AssetTotal.ToString("F4", System.Globalization.CultureInfo.InvariantCulture),
                            Difference = run.Difference.ToString("F4", System.Globalization.CultureInfo.InvariantCulture),
                            BalanceTolerance = run.BalanceTolerance.ToString("F4", System.Globalization.CultureInfo.InvariantCulture),
                            run.Status,
                            run.WarningSummary,
                            run.ErrorSummary
                        };
            ...
            using (var conn = new SqliteConnection(_connectionString))
            {
                ...
                foreach (var row in rows)
                {
                    runs.Add(new BalanceSheetVerificationRun
                    {
                        ...
                        LiabilityTotal = decimal.Parse((string)row.LiabilityTotalText, System.Globalization.CultureInfo.InvariantCulture),
                        AssetTotal = decimal.Parse((string)row.AssetTotalText, System.Globalization.CultureInfo.InvariantCulture),
                        Difference = decimal.Parse((string)row.DifferenceText, System.Globalization.CultureInfo.InvariantCulture),
                        BalanceTolerance = decimal.Parse((string)row.BalanceToleranceText, System.Globalization.CultureInfo.InvariantCulture),
